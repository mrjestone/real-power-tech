...
/**
 * Read a RouterOS list endpoint and normalize empty replies to [] so
 * node-routeros does not crash on `!empty` for no-row responses.
 */
async function safeList(api, path, params = []) {
  try {
...
export async function activateHotspotUser({
  ...
  try {
    ...
    const params = [
      `=name=${mac}`,
      `=password=${mac}`, // CRITICAL: Password must match username for MAC auth
      `=mac-address=${mac}`,
    ...
    try {
      ...
      // If user already exists, remove any active session and recreate the user
      // so the uptime counter resets for the new paid session.
      if (addError.message && addError.message.includes("already have user")) {
        console.log(`ℹ️ User already exists, resetting user for fresh session...`);

        const allUsers = await safeList(api, "/ip/hotspot/user/print", [
          `=.proplist=.id,name`,
        ]);
        const allActive = await safeList(api, "/ip/hotspot/active/print", [
          `=.proplist=.id,user,mac-address`,
        ]);
        ...
        if (existingUser) {
          for (const session of allActive) {
            if (session["user"] === mac || session["mac-address"] === mac) {
              try {
                ...
                console.warn(
                  `⚠️ Could not remove active session:`,
          ...
          try {
            ...
            console.log(`✅ Removed old user: ${existingUser[".id"]}`);
          } catch (deleteError) {
            console.warn(`⚠️ Could not remove old user:`, deleteError.message);
          }
        }

        const recreateResult = await api.write("/ip/hotspot/user/add", params);

        if (Array.isArray(recreateResult) && recreateResult.length > 0 && recreateResult[0].ret) {
          userId = recreateResult[0].ret;
        } else if (typeof recreateResult === "string") {
          userId = recreateResult;
        }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          const allBindings = await safeList(
            api,
...
export async function disconnectHotspotUser(locationId, mac) {
  ...
  try {
    ...
    const activeSessions = await safeList(api, "/ip/hotspot/active/print", [
      "=.proplist=.id,mac-address",
    ]);
    ...
    const users = await safeList(api, "/ip/hotspot/user/print", [
      "=.proplist=.id,name",
    ]);
...
export async function getActiveSession(locationId, mac) {
  ...
  try {
    ...
    const active = await safeList(api, "/ip/hotspot/active/print", [
